...
template <typename T>
MyMatrix<T>::MyMatrix(const MyMatrix<T>& other) : mrows(other.nrows), mcols(other.mcols), m_mat(other.m_mat)
{
}
...
{
	MyMatrix<T> res = MyMatrix(mcols, mrows, 0);
	for (std::size_t i = 0; i < mrows; i++)
	{
		for (std::size_t j = 0; j < mrows; j++)
		{
			res[j][i] = m_mat[i][j];
		}
	}
...
std::vector<T> MyMatrix<T>::gauss_with_selection(std::vector<T> b)
{
	...
	for (std::size_t i = 0; i < mrows - 1; i++)
	{
		...
		this->swapRows(i, indexMaxI);
		T p = b[i];
		b[i] = b[indexMaxI];
		b[indexMaxI] = p;
		for (std::size_t m = i; m < mrows; m++)
		{
			std::size_t k = mrows - 1 + i - m;
			T mI = m_mat[k][i] / maxI;
			for (std::size_t z = i; z < mrows; z++)
			{
				if (k != i)
				{
					m_mat[k][z] -= mI * m_mat[i][z];
				}
				else {
					m_mat[i][z] /= maxI;
				}
	...
	std::vector<T> a(mrows);
	for (std::size_t i = mrows; i >= 1; i--)
	{
		a[i - 1] = b[i - 1] / m_mat[i - 1][i - 1];
		for (std::size_t j = mrows - 1; j > i - 1; j--)
		{
			a[i - 1] -= m_mat[i - 1][j] * a[j] / m_mat[i - 1][i - 1];
		}
	}
	return a;
}
...
{
	MyMatrix m_copy = m_mat; //реалізувати конструктор копіювання!!!
	std::size_t n = mrows;
	std::vector<T> p(n);
	...
	for (std::size_t k = 0; k < n; k++)
	{
		...
		for (int i = k; i < n; i++)//шукаємо елемент з найбільшим модулем
		//в стовпці к для всіх рядків к та більше
		{
			if (fabs(m_copy[i][k]) > p)
			{
				p = m_mat[i][k];
				indexM = i;
			}
